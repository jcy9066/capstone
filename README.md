...
## Pi?GPU ?? ??? LiDAR dry-run ??

? ??? Raspberry Pi ??? H.264? RPLIDAR /scan? GPU ??? ????, LiDAR ????? **?? ?? ??**? ???? ????. ?? ??? API? ?? ????? ?? ??? ???? ???.

- ?? /cmd_vel publish ??
- GPIO, serial, Pico W, MDD10A, ?? controller ?? ??
- WebSocket?? move ??? ?? ???? ??
- MOTOR_OUTPUT_ENABLED? ?? ??? ?? false? ????, true ?? ??? ?? ??? ???? ??
- odometry, ?? ?? ??, SLAM/Nav2 ????? ?? ???

LaserScan? ??(-20?~+20?), ??(+20?~+90?), ??(-90?~-20?)?? ???. ? ??? ?? ?? range? ?? 10 percentile??. ?? ??? ???? ?? beam? ?? ???? ?? ?? ?? beam? ??? ??? ??? ? ????.
...
NAV_DRY_RUN_ENABLED=true? ?? ?? ??? ????. scan? ??? WAITING_FOR_SCAN, ??? ?? ? NAV_SCAN_TIMEOUT_SEC? ??? STOP? SCAN_TIMEOUT? ????. stop/slow ?? ??? ?? ??(<=)??.

### GPU ??: Windows ???? WSL2 Ubuntu

Windows Python? ??? WSL Ubuntu??? ???? ??? ????. ? ?? ??? ?? WSL ??? ??? ??.

~~~bash
cd '/mnt/j/???_??/git clone/dabom_capstone'
...
WSL? Windows ???/Tailscale ??? Pi? http://100.100.248.122:21063? ??? ? ??? ??? ????.

### Raspberry Pi ???

Pi ??? ???? ??? ????. ????? ????? .env? ??, rpicam-vid? inline H.264? ?? HTTP /stream/h264? chunked upload ??. RTSP ??? ??? ???.
...
?? ????FPS? 640x480, 15??, ???? ????? ???? STREAM_RETRY_SEC(?? 3?) ? ?? ????. Ctrl+C ?? SIGTERM? ???? curl ?? ????? ????.

### Raspberry Pi LiDAR only

RPLIDAR A1M8? /dev/ttyUSB0, 115200 baud? /scan? publish???? ??? ?, SLAM?fake odom ?? bridge? ??? ? ??.
...
?? map_bridge? NaN/Infinity range? JSON null? ???, max_scan_points? payload? ???, HTTP timeout???? ??? throttled log?? ???. ?? ?? ??? ROS node? ????? ???.

?? mapping ??? ??? ???? ?? mapping.launch.py? ????. ?? ?? ??? ????? fake odom/SLAM? ?? dry-run ??? ?? ???? ???? ???.
...
~~~json
{
  ...
  "linear_x": 0.0,
  "angular_z": 0.65,
  "front_distance_m": 0.32,
  "left_distance_m": 1.4,
  "right_distance_m": 0.72,
  "scan_age_sec": 0.15,
  "updated_at": 1780000000.0
}
~~~

action? FORWARD, SLOW_AVOID, TURN_LEFT, TURN_RIGHT, STOP, WAITING_FOR_SCAN ? ???. ?? ?? ?????? dry-run ??? ??? ???? ???. ???? ????? http://100.100.248.122:21063/?? ?? ?? ??? ????, navigation API? ? curl ?? ???? ??? ???? ????.
